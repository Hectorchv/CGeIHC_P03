...
int main()
{
	...
	*/
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(00.0f, -0.01f, -5.125f));
		model = glm::rotate(model, glm::radians(180.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.3f, 0.3f, 0.3f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glUniformMatrix4fv(uniformView, 1, GL_FALSE, glm::value_ptr(camera.calculateViewMatrix()));
		...
		//printf("x:%f, y:%f, z:%f\n", mainWindow.getrotax()-(floor(mainWindow.getrotax()/360)*360), mainWindow.getrotay()-(floor(mainWindow.getrotay()/360)*360), mainWindow.getrotaz()-(floor(mainWindow.getrotaz()/360)*360));
		glUseProgram(0);
